...
export default function Contact() {
    ...
    const handleSubmit = async (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        setError('')
        setSuccess('')
        if (!getFullname(userinfo) || !userinfo.emailaddr || !userinfo.message) {
        ...
        const res = await fetch('/api/contact', {
            ...
            body: JSON.stringify({ name : getFullname(userinfo), email : userinfo.emailaddr, message : userinfo.message }),
        })
  ...
  return (
    <div className="isolate bg-gray-900 px-6 py-24 sm:py-32 lg:px-8">
      ...
      <div className="mx-auto max-w-2xl text-center">
        ...
        <p className="mt-2 text-lg/8 text-gray-400">You can receive friendly service 24 hours a day.</p>
      </div>
          ...
          <div className="sm:col-span-2">
            ...
            <div className="mt-2.5">
              <input
                ...
                type="email"
                autoComplete="email"
                onChange={(e) => updateUserinfo('emailaddr', e.target.value)}
                className="block w-full rounded-md bg-white/5 px-3.5 py-2 text-base text-white outline-1 -outline-offset-1 outline-white/10 placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500"
              />
            </div>
          </div>
          ...
          <div className="sm:col-span-2">
            ...
            <div className="mt-2.5">
              <textarea
                ...
                name="message"
                rows={4}
                onChange={(e) => updateUserinfo('message', e.target.value)}
                className="block w-full rounded-md bg-white/5 px-3.5 py-2 text-base text-white outline-1 -outline-offset-1 outline-white/10 placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500"
                defaultValue={''}
              />
            </div>
